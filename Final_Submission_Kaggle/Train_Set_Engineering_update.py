...
def training_data_processing(path):
    """
    Purpose: 
    ...
    # Here we rename the ID column to account.id so that we can merge dataframes together using the account.id as the primary key. 
    training_df = training_df.rename(columns={"ID": "account.id"})
    # We can now merge the training_df with the accounts_df using the account.id as the primary key
    training_accounts = pd.merge(training_df, data['accounts_df'], on='account.id', how='left')
    # We can now merge training_accounts with the subscriptions_df using the account.id as the primary key
    training_accounts_subscriptions = pd.merge(training_accounts, data['subscriptions_df'], on='account.id', how='left')
    ...
    # For this reason, we can take the aggregate of each feature when we are merging so that we keep the training rows at the same length (6491 rows)
    ...
    # Now we can create an aggregated dataframe by grouping by the account.id and aggregating the features using the aggregation_dictionary_1. 
    training_aggregate_1 = training_accounts_subscriptions.groupby('account.id').agg(aggregation_dictionary_1).reset_index()
    # Now we can merge with the tickets dataframe 
    training_aggregate_tickets = pd.merge(training_aggregate_1, data['tickets_df'], on='account.id', how='left')
    # We need to drop the season_y column due to redundancy 
    training_aggregate_tickets.drop(columns=['season_y'], inplace=True)
    # We need to also rename season_x to season
    training_aggregate_tickets.rename(columns={'season_x': 'season'}, inplace=True)
    ...
    # We can aggregate the training_aggregate_tickets dataframe like last time, except this time we use the aggregation_dictionary_2
    training_aggregate_2 = training_aggregate_tickets.groupby('account.id').agg(aggregation_dictionary_2).reset_index()
    # We can merge with the zipcodes dataframe now by using the left_on and right_on parameters
    training_aggregate_zipcodes = pd.merge(training_aggregate_2, data['zipcodes_df'], left_on='billing.city', right_on='City', how='left')
    ...
    training_aggregate_3 = training_aggregate_zipcodes.groupby('account.id').agg(aggregation_dictionary_3).reset_index()
    ...
    # Here we merge the training_aggregate_3 with the concerts_df using the set as means of merging.
    training_aggregate_concerts = pd.merge(training_aggregate_3, concerts_df, on= 'set', how='left')
    ...
    # Like before, we can aggregate the training_aggregate_concerts dataframe by the account.id and use the aggregation_dictionary_final.
    training_aggregate_3 = training_aggregate_concerts.groupby('account.id').agg(aggregation_dictionary_final).reset_index()
    # Finally, we can merge the training_aggregate_3 with the concerts2_df using the set as means of merging.
    training_aggregate_final = pd.merge(training_aggregate_3, data['concerts2_df'] [['set', 'concert.name', 'season', 'who']], on='set', how='left')
    
    # Return the final dataframe
    return training_aggregate_final

# Now we need to make a function that is responsible for filling in any missing values within the trainign and training dataframes. 
def missing_values_filler(dataframe):
    ...
    'no.seats', 'multiple.tickets', 'package', 'section', 'TotalWages', 'set', 'season_x', 'season_y', 'State', 'billing.city', 'concert.name_x', 'who_x', 'what',
    'location', 'concert.name_y', 'season', 'who_y']
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Enter Proper Paths to Files")
        sys.exit(1)

    path = sys.argv[1]
    result = training_data_processing(path)
    result = missing_values_filler(result)
    # Save the result dataframe in the same directory as the input data
    result.to_csv(f"{path}/training_aggregate_final.csv")
